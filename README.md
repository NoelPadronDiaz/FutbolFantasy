...
- **Valor real**: cada jugador muestra su valor de mercado actual en LaLiga Fantasy (`real_price`), junto al precio al que lo compraste. Se actualiza solo, una vez al día.
...
- **Valor real**: `api/cron/update-real-prices.ts` corre cada noche vía [Vercel Cron](https://vercel.com/docs/cron-jobs) (`vercel.json`, `30 0 * * *` UTC ≈ 01:30 hora española en invierno; en verano, por el cambio de horario, se ejecuta a las 02:30). Descarga la lista pública de jugadores de LaLiga Fantasy (`fantasy-api.llt-services.com`, sin login) y actualiza `real_price` en los jugadores cuyo `name` coincide (sin tildes/mayúsculas) con el `nickname` de la API. Protegido con `CRON_SECRET` (Vercel lo envía automáticamente como cabecera `Authorization` al invocar el cron).

Angular no puede conectarse directamente a Postgres desde el navegador, así que todo el acceso a datos pasa por `api/players/index.ts` (listar/crear vía GET/POST, y vender/recuperar/editar/eliminar vía PATCH/DELETE con `?id=`), `api/auth/*` (`register`, `login`, `logout`, `me`) y `api/cron/update-real-prices.ts`.
...
2. Genera un `AUTH_SECRET` (para firmar las cookies de sesión) y un `CRON_SECRET` (para proteger el endpoint del cron), y añádelos también a `.env`:
   ```bash
...
Al desplegar en Vercel (`vercel deploy` o mediante un push con la integración de Git activada), configura `DATABASE_URL`, `AUTH_SECRET` y `CRON_SECRET` como variables de entorno del proyecto en Vercel (Settings → Environment Variables, para Production/Preview/Development) — `DATABASE_URL` ya estará configurada automáticamente si la base de datos se creó desde la integración de Vercel Postgres/Neon, pero `AUTH_SECRET`/`CRON_SECRET` hay que añadirlas a mano (usa valores distintos a los de tu `.env` local). Vercel construye el frontend Angular y despliega las funciones de `api/` en el mismo dominio, así que no hace falta configurar CORS ni URLs separadas. El cron job (`vercel.json` → `crons`) se activa automáticamente al desplegar; en el plan gratuito de Vercel se ejecuta dentro de la hora programada, no al minuto exacto.
